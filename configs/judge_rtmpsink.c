...
int main(int argc, char *argv[]) {
  ...
  GstElement *mixer, *output_converter, *encoder, *muxer, *output_sink;
  ...
  judge_stream_caps = gst_caps_new_simple("video/x-raw","width", G_TYPE_INT, 212, \
  ...
  g_object_set(main_stream, "location", "rtmp://192.168.1.124:1935/yanked/stream-meu", NULL);
  /* g_object_set(main_stream, "location", "rtmp://192.168.1.124:1935/yanked/stream-11111111-1111-1111-1111-111111111111", NULL); */
  g_object_set(judge1_stream, "location", "rtmp://192.168.1.124:1935/yanked/stream-counter", NULL);
  g_object_set(judge2_stream, "location", "rtmp://192.168.1.124:1935/yanked/stream-counter", NULL);
  g_object_set(judge3_stream, "location", "rtmp://192.168.1.124:1935/yanked/stream-counter", NULL);
  ...
  g_object_set(main_stream_videobox, "left", 106, "right", 106, "top", 0, "bottom", 0, NULL);
  ...
  if(!gst_element_link(output_converter, encoder)) {
    return print_error_and_exit("Could not link the output converter to the h264 encoder.\n", &data);
  }

  if(!gst_element_link(encoder, muxer)) {
    return print_error_and_exit("Could not link the h264 encoder to the FLV muxer.\n", &data);
  }

  if(!gst_element_link(muxer, output_sink)) {
    return print_error_and_exit("Could not link the FLV muxer to the output sink.\n", &data);
  }
  ...
  if(!gst_element_link(main_source_sink.sink, main_stream_videobox)) {
  ...
  /* Position the judges */
  g_object_set(judge1_stream_video_pad, "xpos", 428, "ypos", 0, "zorder", 100, NULL);
  g_object_set(judge2_stream_video_pad, "xpos", 428, "ypos", 120, "zorder", 100, NULL);
  g_object_set(judge3_stream_video_pad, "xpos", 428, "ypos", 240, "zorder", 100, NULL);
