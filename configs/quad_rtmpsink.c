...
int main(int argc, char *argv[]) {
  ...
  GstElement *output_converter, *encoder, *muxer, *output_sink;
  ...
  /* static things */
  capabilities = gst_caps_new_simple("video/x-raw", "width", G_TYPE_INT, 320, \
  ...
  if(!data.pipeline || !rtmp_source1 || !rtmp_source2 || !rtmp_source3 || !rtmp_source4 || \
     ...
     !bottom_right.source || !bottom_right.sink || !encoder || !muxer) {
  ...
  gst_bin_add_many(GST_BIN(data.pipeline), rtmp_source1, rtmp_source2, rtmp_source3, rtmp_source4, \
		   ...
		   mixer, output_converter, output_sink, top_left.source, top_left.sink, \
		   top_right.source, top_right.sink, bottom_left.source, bottom_left.sink, \
		   bottom_right.source, bottom_right.sink, muxer, encoder, NULL);

  g_object_set(rtmp_source1, "location", "rtmp://192.168.1.124:1935/yanked/stream-counter", NULL);
  g_object_set(rtmp_source2, "location", "rtmp://192.168.1.124:1935/yanked/stream-counter", NULL);
  g_object_set(rtmp_source3, "location", "rtmp://192.168.1.124:1935/yanked/stream-meu", NULL);
  g_object_set(rtmp_source4, "location", "rtmp://192.168.1.124:1935/yanked/stream-meu", NULL);
  ...
  if(!gst_element_link(muxer, output_sink)) {
    g_printerr("Could not link the FLV muxer to the output sink.\n");
    gst_object_unref(data.pipeline);
    return -1;
  }
