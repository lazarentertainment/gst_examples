...
int main(int argc, char *argv[]) {
  GstElement *source1, *converter2, *sink;
  ...
  if(!data.pipeline || !source1 || !data.sink || !converter2 || !data.source || !sink || !encoder || !muxer) {
    g_printerr("Could not build an element\n");
    return -1;
  }

  gst_bin_add_many(GST_BIN(data.pipeline), source1, data.sink, converter2, data.source, sink, encoder, muxer, NULL);
  ...
  if(!gst_element_link(encoder, muxer)) {
    g_printerr("Could not link the encoder to the muxer.\n");
    gst_object_unref(data.pipeline);
    return -1;
  }

  if(!gst_element_link(muxer, sink)) {
    g_printerr("Could not link the muxer to the RTMP sink.\n");
    gst_object_unref(data.pipeline);
    return -1;
  }
