...
int main(int argc, char *argv[]) {
  ...
  GstElement *encoder, *muxer, *output_converter;
  GstBus *bus;
  GstCaps *caps;
  ...
  if(!data.pipeline || !source1 || !source2 || !left_stream.sink || !left_videobox || \
     ...
     !mixer || !output_converter || !encoder || !muxer || !sink) {
  ...
  gst_bin_add_many(GST_BIN(data.pipeline), source1, left_stream.sink, left_videobox,\
		   ...
		   right_videobox, mixer, output_converter, encoder, muxer, sink, NULL);

  g_object_set(source1, "location", "rtmp://192.168.1.124:1935/yanked/stream-counter", NULL);
  ...
  if(!gst_element_link(left_stream.sink, left_videobox)) {
  ...
  if(!gst_element_link(right_stream.sink, right_videobox)) {
  ...
  if(!gst_element_link(output_converter, encoder)) {
    g_printerr("Could not link output converter to the h264 encoder.\n");
    gst_object_unref(data.pipeline);
    return -1;
  }

  if(!gst_element_link(encoder, muxer)) {
    g_printerr("Could not link the encoder to the FLV muxer.\n");
    gst_object_unref(data.pipeline);
    return -1;
  }
